            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
                    <caption class="bg-white p-5 text-left text-lg font-semibold text-gray-900 dark:bg-gray-800 dark:text-white">
                        <button class="rounded-lg bg-blue-700 px-3 py-2 text-center text-xs font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button" x-on:click="modalCreate">Add City</button>
                    </caption>
                        ...
                        @foreach ($cities as $city)
                            <tr class="border-b bg-white dark:border-gray-700 dark:bg-gray-900">
                                ...
                                <td class="flex gap-2 px-6 py-4">
                                    <button class="font-medium text-blue-600 hover:underline dark:text-blue-500" type="button" x-on:click="modalEdit('{{ $city->id }}')">Edit</button>
                                    @can('delete', $city)
                                        <button class="font-medium text-red-600 hover:underline dark:text-red-500" type="button" x-on:click.prevent="if(confirm('Are You Sure?')) $wire.destroy('{{ $city->id }}');">Delete</button>
                                    @endcan
                                </td>
                            </tr>
                        @endforeach
            ...
            <div class="relative rounded-lg bg-white shadow dark:bg-gray-700">
                ...
                <div class="px-6 py-6 lg:px-8">
                    <form class="space-y-6" wire:submit.prevent="{{ $submit }}">
                        <div>
                            <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white" for="title">City Name</label>
                            <x-input :required="true" class="block w-full" placeholder="City Name" type="text" wire:model="form.name" />
                        </div>

                        <div wire:ignore>
                            <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white" for="title">Country</label>
                            <select class="multiselect" required wire:model="form.country_id">
                                <option value=""></option>
